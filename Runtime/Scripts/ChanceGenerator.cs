...
namespace ChanceGen
{
    ...
    public sealed partial class ChanceGenerator
    {
        ...
        public int MaxWalkValue
        {
            get
            {
                Assert.IsTrue(_isNodeAccessAllowed >= 3,
                    "Cannot access walk value until post walk generation started!");
                return _maxWalkValue;
            }

            private set => _maxWalkValue = value;
        }

        private int _maxWalkValue;
        ...
        public async Task<ReadOnlyMemory<Node>> Generate(CancellationToken requiredToken)
        {
            ...
            _maxWalkValue = await Task.Run(() => WalkNodesAndApplyValues(found, Generated, _branchRequirement),
                Application.exitCancellationToken); // walk nodes

            _isNodeAccessAllowed = 3;
